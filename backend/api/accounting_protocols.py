...
async def refund_event(payload: RefundEventCreate, current: User = Admin):
    try:
        return await accounting_protocols.record_refund_accounting_event(
...
async def revenue_recognition(payload: RevenueRecognitionCreate, current: User = Admin):
    try:
        return await accounting_protocols.prepare_revenue_recognition(
            actor_id=current.id, **payload.model_dump()
        )
    except (LookupError, ValueError) as exc:
        _raise(exc)
...
async def missing_document(payload: MissingDocumentCreate, current: User = Admin):
    try:
        return await accounting_protocols.register_missing_supporting_document(
            actor_id=current.id, **payload.model_dump()
        )
    except (LookupError, ValueError) as exc:
        _raise(exc)
...
async def resolve_missing_document(
    missing_document_id: str,
    payload: MissingDocumentResolve,
    current: User = Admin,
):
    try:
        return await accounting_protocols.resolve_missing_supporting_document(
            actor_id=current.id,
...
async def fec_readiness(period_id: str, current: User = Admin):
    try:
        return await accounting_protocols.fec_readiness_gate(period_id=period_id)
    except (LookupError, ValueError) as exc:
        _raise(exc)
...
async def retention(payload: RetentionPolicyCreate, current: User = Admin):
    try:
        return await accounting_protocols.register_financial_retention_policy(
            actor_id=current.id, **payload.model_dump()
        )
    except (LookupError, ValueError) as exc:
        _raise(exc)
