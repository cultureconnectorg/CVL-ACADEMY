...
from services.economy_workbook_runtime import OFFER_PHASE, SHEETS, set_phase_state

router = APIRouter(prefix="/master/economy-workbook", tags=["master-registry"])
...
async def list_economy_workbook_sheets(current: User = Admin):
    ...
    return [{"sheet": sheet, "runtime_rows": counts.get(sheet, 0)} for sheet in SHEETS]
...
async def economy_workbook_sheet(
    ...
    if sheet not in SHEETS:
        raise HTTPException(status_code=404, detail="unknown Economy workbook sheet")
    return await db.academy_economy_workbook_rows.find(
        {"sheet": sheet},
        {"_id": 0},
    ).sort("csv_row", 1).limit(limit).to_list(limit)
...
async def economy_workbook_row(
    ...
    if not row:
        raise HTTPException(status_code=404, detail="Economy workbook row not found")
    return row


@router.get("/phases")
async def economy_phase_states(current: User = Admin):
    states = await db.academy_economy_phase_state.find({}, {"_id": 0}).to_list(10)
    by_phase = {state["phase"]: state for state in states}
    return {
        "offer_phase": OFFER_PHASE,
        "states": {
            phase: by_phase.get(phase, {"phase": phase, "active": False})
