...
@router.get("/")
async def root():
    agent_factory_status = agent_factory.remote_status()
    return {
        "app": "CVLN Academy OS",
        "version": "0.1",
        ...
        # Backward-compatible legacy field. It means configured, not active.
        "agent_factory_remote": agent_factory.is_remote_enabled(),
        "agent_factory_remote_configured": agent_factory_status["configured"],
        "agent_factory_remote_active": agent_factory_status["active"],
        "ai_inference": agent_factory.inference_status(),
    }
...
async def ready(request: Request):
    ...
    payload = {
        ...
        "frek_core_remote": frek_core.is_remote_enabled(),
        # Keep readiness cheap: GPU discovery has its own /accelerators/status probe.
        "agent_factory_remote": agent_factory.is_remote_enabled(),
        "agent_factory_remote_configured": agent_factory_status["configured"],
        "agent_factory_remote_active": agent_factory_status["active"],
    }
