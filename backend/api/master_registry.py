...
async def list_protocol_controls(
    domain: str | None = None,
    limit: int = Query(default=227, ge=1, le=227),
    current: User = Admin,
):
...
async def execute_protocol_control(
    control_id: str,
    payload: ProtocolExecutionPayload,
    current: User = Admin,
):
    row = await db.academy_protocol_controls.find_one(
        {"control_id": control_id}, {"_id": 0}
...
async def requirement_summary(current: User = Admin):
    ...
    verified = await db.academy_requirement_registry.count_documents(
        {"verified": True}
    )
