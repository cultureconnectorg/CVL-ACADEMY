...
async def legal_basis(payload: LegalBasisCreate, current: User = Admin):
    try: return await privacy_protocols.register_legal_basis(actor_id=current.id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/processing-activities/{activity_id}/purpose-assessment")
async def purpose(activity_id: str, payload: PurposeAssessment, current: User = Admin):
    try: return await privacy_protocols.assess_purpose_limitation(actor_id=current.id, processing_activity_id=activity_id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/processing-activities/{activity_id}/minimisation")
async def minimisation(activity_id: str, payload: MinimisationAssessment, current: User = Admin):
    try: return await privacy_protocols.assess_data_minimisation(actor_id=current.id, processing_activity_id=activity_id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/consents/{consent_id}/frek-proof")
async def consent_frek(consent_id: str, payload: FrekConsentProof, current: User = Admin):
    try: return await privacy_protocols.record_consent_frek_proof(actor_id=current.id, consent_id=consent_id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/requests")
async def privacy_request(payload: PrivacyRequestCreate, current: User = Admin):
    try: return await privacy_protocols.create_restriction_or_objection(actor_id=current.id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.patch("/requests/{request_id}")
async def privacy_request_transition(request_id: str, payload: PrivacyRequestTransition, current: User = Admin):
    try: return await privacy_protocols.transition_restriction_or_objection(actor_id=current.id, request_id=request_id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/pseudonymisation")
async def pseudonymisation(payload: PseudonymisationCreate, current: User = Admin):
    try: return await privacy_protocols.register_pseudonymisation_policy(actor_id=current.id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/residency")
async def residency(payload: ResidencyCreate, current: User = Admin):
    try: return await privacy_protocols.register_data_residency_rule(actor_id=current.id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/breach-investigations")
async def breach(payload: BreachCreate, current: User = Admin):
    try: return await privacy_protocols.open_breach_investigation(actor_id=current.id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/breach-investigations/{investigation_id}/findings")
async def breach_finding(investigation_id: str, payload: BreachFinding, current: User = Admin):
    try: return await privacy_protocols.record_breach_finding(actor_id=current.id, investigation_id=investigation_id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/minor-assessments")
async def minor(payload: MinorAssessment, current: User = Admin):
    try: return await privacy_protocols.assess_minor_user(actor_id=current.id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/jurisdiction-packs")
async def jurisdiction_pack(payload: JurisdictionPack, current: User = Admin):
    try: return await privacy_protocols.create_jurisdiction_pack(actor_id=current.id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)

@router.post("/evidence-packs")
async def evidence_pack(payload: PrivacyEvidencePack, current: User = Admin):
    try: return await privacy_protocols.create_privacy_evidence_pack(actor_id=current.id, **payload.model_dump())
    except (LookupError, ValueError) as exc: _raise(exc)
