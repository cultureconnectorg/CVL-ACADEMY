"""Server-side import of the real `docs/kor/korXX/` tree into
`db.kor_resources` — the one new, additive collection this whole
package writes to. **Never writes to `db.formations`, `seed_data.py`,
`db.progress`, or any legacy route.**

Unlike FMS (a ZIP a human uploads), the KORA corpus already lives
unpacked in this repo (`docs/kor/`) — so "import" here means "scan the
real filesystem tree and persist a structured read model of it",
triggered by an admin action, not a file upload — same shape as
`klt_canonical/import_pipeline.py`. Re-running it is idempotent (upsert
by `source_file` + `canonical_version`) — it never duplicates.
"""
...
from .models import KOR_CANONICAL_VERSION_CURRENT, KorCanonicalImportResult
from .parser import (classify_resource_type, extract_heading_title,
                     formation_code_from_path, parse_module_file,
                     parse_skill_registry)
from .provenance import default_docs_dir, list_real_files, store_kor_provenance

COLLECTION = "kor_resources"
        ...
        payload.update(
            {
                ...
                "prerequisite_module_code": parsed["prerequisite_module_code"],
                "assessment_level": parsed["assessment_level"],
                "kora_dependency": parsed["kora_dependency"],
...
async def import_kor_docs(
    ...
    *,
    ...
    provenance_records, _inserted, _updated = await store_kor_provenance(
    ...
    await db.kor_imports.insert_one(
        {
            "id": import_id,
            "canonical_version": canonical_version,
    ...
    return KorCanonicalImportResult(
        import_id=import_id,
        docs_dir=str(docs_dir),
        total_files=total_files,
        parsed_count=parsed_count,
