...
async def record_refund_accounting_event(
    *,
    ...
    if amount_cents <= 0 or not refund_ref.strip() or not refs:
        raise ValueError(
            "refund accounting event requires positive amount, refund ref and evidence"
        )
    payment = await db.payments.find_one({"id": payment_id}, {"_id": 0})
    if not payment:
        raise LookupError("payment not found")
    if credit_note_id:
        note = await db.accounting_credit_notes.find_one(
            {"id": credit_note_id}, {"_id": 0}
        )
        if not note:
    ...
    existing = await db.accounting_events.find_one(
        {"source_type": "REFUND", "source_ref": refund_ref.strip()}, {"_id": 0}
    )
    if existing:
        return existing
    row = {
        ...
        "event_type": "REFUND",
        "source_type": "REFUND",
        "source_ref": refund_ref.strip(),
        "payment_id": payment_id,
        "amount_cents": int(amount_cents),
        "currency": currency.strip().upper(),
        "credit_note_id": credit_note_id,
        "evidence_refs": refs,
        "posting_status": "UNMAPPED",
        "created_by": actor_id,
...
async def prepare_revenue_recognition(
    *,
    ...
    if (
        ...
        or not recognition_date.strip()
        or not refs
    ):
        raise ValueError(
            "revenue recognition preparation requires source, amount, date and evidence"
        )
    ...
    row = {
        ...
        "amount_cents": int(amount_cents),
        "currency": currency.strip().upper(),
        "recognition_date": recognition_date.strip(),
...
async def register_missing_supporting_document(
    *,
    ...
    refs = _refs(evidence_refs)
    required = (expected_document_type, source_ref, owner, due_at)
    if not all(str(v).strip() for v in required) or not refs:
        raise ValueError(
            "missing-document record requires type, source, owner, due date and evidence"
        )
    row = {
        "id": _id("MISSDOC"),
        "period_id": period_id,
...
async def resolve_missing_supporting_document(
    *,
    ...
    row = await db.accounting_missing_documents.find_one(
        {"id": missing_document_id}, {"_id": 0}
    )
    doc = await db.accounting_supporting_documents.find_one(
        {"id": supporting_document_id}, {"_id": 0}
    )
    ...
    update = {
        "status": "RESOLVED",
        "resolved_document_id": supporting_document_id,
        "resolution_evidence_refs": refs,
        "resolved_by": actor_id,
        "resolved_at": now,
        "updated_at": now,
    }
    await db.accounting_missing_documents.update_one(
        {"id": missing_document_id, "status": "OPEN"}, {"$set": update}
    )
    return {**row, **update}
...
async def fec_readiness_gate(*, period_id: str) -> Dict[str, Any]:
    ...
    if not await db.accounting_periods.find_one({"id": period_id}):
        raise LookupError("accounting period not found")
    missing_docs = await db.accounting_missing_documents.find(
        {"period_id": period_id, "status": "OPEN"}, {"_id": 0}
    ).to_list(10000)
    unmapped = await db.accounting_events.find(
        {"period_id": period_id, "posting_status": "UNMAPPED"}, {"_id": 0}
    ).to_list(10000)
    mapping_errors = await db.accounting_period_anomalies.find(
        {"period_id": period_id, "status": "OPEN"}, {"_id": 0}
    ).to_list(10000)
...
async def register_financial_retention_policy(
    *,
    ...
    if retention_days < 0 or not record_type.strip() or not trigger.strip() or not refs:
        raise ValueError(
            "financial retention requires record type, retention, trigger and evidence"
        )
    policy = await policy_registry.require_effective_version(policy_version_id)
    ...
    await db.accounting_retention_policies.update_one(
        {"record_type": row["record_type"], "status": "ACTIVE"},
        {"$set": row},
        upsert=True,
    )
    await governance.audit_event(
        event_type="accounting.retention_policy.registered",
        actor_id=actor_id,
        resource_type="accounting_retention_policy",
        resource_id=row["id"],
        payload={
