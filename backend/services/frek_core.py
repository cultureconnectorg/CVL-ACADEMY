...
FREK_CORE_BASE_URL = os.environ.get("FREK_CORE_BASE_URL")  # optional; when set, remote calls are attempted
...
VALID_SIGNALS = {
    "FREK-TIME", "FREK-WORK", "FREK-SCORE",
    "FREK-LINK", "FREK-CERT", "FREK-CONTRIB",
    "FREK-SHARE", "FREK-MISSION",
}
...
class FrekCoreClient:
    ...
    async def _remote_post(self, path: str, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if not self.is_remote_enabled():
            return None
        try:
            headers = {"Authorization": f"Bearer {FREK_CORE_API_KEY}"} if FREK_CORE_API_KEY else {}
            async with httpx.AsyncClient(timeout=8.0) as c:
                r = await c.post(f"{FREK_CORE_BASE_URL}{path}", json=payload, headers=headers)
    ...
    async def emit_signal(self, user_id: str, signal: str, meta: Optional[Dict[str, Any]] = None) -> None:
        ...
        await self._remote_post("/signal", {
            "user_id": user_id, "signal": signal, "meta": meta or {}
        })

    async def issue_proof(self, user_id: str, kind: str, meta: Optional[Dict[str, Any]] = None) -> str:
        """Issue a portable proof (badge/cert). Returns a proof reference id."""
        remote = await self._remote_post("/proof", {
