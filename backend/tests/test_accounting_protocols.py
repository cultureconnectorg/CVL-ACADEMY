...
@pytest.mark.asyncio
async def test_refund_accounting_event_is_idempotent_and_can_bind_credit_note(
    accounting_protocol_db,
):
    await accounting_protocol_db.payments.insert_one({"id": "PAY-1", "status": "paid"})
    await accounting_protocol_db.accounting_credit_notes.insert_one(
        {"id": "CN-1", "amount_cents": 5000}
    )
...
async def test_revenue_recognition_is_prepared_not_validated(accounting_protocol_db):
    ...
    row = await accounting_protocols.prepare_revenue_recognition(
        ...
        amount_cents=140000,
        currency="EUR",
        recognition_date="2026-09-11",
        policy_version_id=policy["id"],
        dimensions={"formation_code": "FMS-A"},
        evidence_refs=["INV-1"],
    )
...
@pytest.mark.asyncio
async def test_missing_supporting_document_must_resolve_to_registered_document(
    accounting_protocol_db,
):
    await accounting_protocol_db.accounting_periods.insert_one(
        {"id": "PER-1", "status": "OPEN"}
    )
    missing = await accounting_protocols.register_missing_supporting_document(
        ...
        owner="ops",
        due_at="2026-09-20",
        evidence_refs=["RECON-1"],
    )
    with pytest.raises(LookupError):
        await accounting_protocols.resolve_missing_supporting_document(
            actor_id="accountant",
            missing_document_id=missing["id"],
            supporting_document_id="UNKNOWN",
            evidence_refs=["FOLLOWUP"],
        )
    await accounting_protocol_db.accounting_supporting_documents.insert_one(
        {"id": "DOC-1"}
    )
...
@pytest.mark.asyncio
async def test_fec_readiness_never_claims_fec_generation_or_legal_compliance(
    accounting_protocol_db,
):
    await accounting_protocol_db.accounting_periods.insert_one(
        {"id": "PER-1", "status": "OPEN"}
    ...
    await accounting_protocol_db.accounting_missing_documents.insert_one(
        {"id": "M-1", "period_id": "PER-1", "status": "OPEN"}
    )
    gate = await accounting_protocols.fec_readiness_gate(period_id="PER-1")
