...
from services.cvln_wallet import CVLNWalletAmbiguousResult, CVLNWalletClient
...
async def test_create_order_snapshots_economy_price_and_wallet_rate(
    ...
    assert order["economy_requirement_id"] == "ACA-ECO-0013"
    assert order["wallet_entity_id"] == "ent_cvln_academy"
    stored = await commercial_db.commercial_orders.find_one(
        {"order_id": order["order_id"]}, {"_id": 0}
    )
    assert stored["pricing_snapshot"]["source_sheet"] == "Mapping_812"
...
async def test_wallet_payment_grants_exact_entitlement_once(
    ...
    async def fake_charge(frek_id, amount_cc, note, idempotency_key):
    ...
    assert len(calls) == 1
    assert calls[0][0] == "FREK-001"
    assert calls[0][1] == 660.0
    assert calls[0][3].startswith("pay_")
    assert calls[0][3] == paid["payment_attempt_id"]
    ...
    assert entitlement["source_order_id"] == order["order_id"]
...
async def test_ambiguous_wallet_outcome_is_never_auto_retried(
    ...
    async def ambiguous_charge(frek_id, amount_cc, note, idempotency_key):
        nonlocal calls
        calls += 1
    ...
    assert stored["status"] == "REQUIRES_REVIEW"
    assert stored["review_reason"] == "WALLET_OUTCOME_AMBIGUOUS"
...
async def test_internal_economy_rows_are_not_payment_gated(
    commercial_db, learner, monkeypatch
):
    monkeypatch.setenv("ACADEMY_COMMERCIAL_ENTITLEMENTS_ENFORCED", "true")
    assert await commercial_module.has_commercial_access(learner.id, "KLT-09") is True
