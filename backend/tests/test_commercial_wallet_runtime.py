...
from services.cvln_wallet import CVLNWalletAmbiguousResult
...
async def test_wallet_payment_grants_exact_entitlement_once(
    ...
    async def fake_charge(frek_id, amount_cc, note):
        calls.append((frek_id, amount_cc, note))
...
async def test_ambiguous_wallet_outcome_is_never_auto_retried(
    ...
    async def ambiguous_charge(frek_id, amount_cc, note):
        nonlocal calls
        calls += 1
