...
def test_all_18_unit_economics_products_match_commerce_runtime():
    ...
    mapped_offers = [offer for offer in OFFERS if offer.offer_id in UNIT_OFFER_PRODUCT]
    ...
    for offer in mapped_offers:
        ...
        assert decision["computed_margin_pct"] >= decision["margin_floor_pct"]
...
async def test_full_workbook_import_is_line_complete_and_idempotent():
    ...
    assert await db.academy_economy_workbook_rows.count_documents({}) == expected_total
...
@pytest.mark.asyncio
async def test_checkout_guard_blocks_phase_before_low_level_checkout(monkeypatch):
    client = AsyncMongoMockClient()
    ...
    monkeypatch.setattr(economy_checkout, "create_payment_checkout", fake_checkout)
