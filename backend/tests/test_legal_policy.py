...
# ---------------------------------------------------------------------------
# RECONCILE-2 Groupe 5 (2026-09-14): main independently created a *different*
# module also named `legal_policy` -- the root-level `backend/legal_policy.py`
# (the versioned legal-acceptance bundle shown at onboarding: CGU/règlement/
# charte IA/confidentialité), completely unrelated to `services.legal_policy`
# above (the Authority Policy Engine's ALLOW/ESCALATE/DENY review-requirement
# gate). Both files happened to get the same test filename
# (`test_legal_policy.py`) independently on each branch. No symbol collision:
# this import extracts named constants rather than binding the module name
# `legal_policy` itself (already bound above, to the *other* module, by the
# `from services import ... legal_policy ...` at the top of this file).
# ---------------------------------------------------------------------------
