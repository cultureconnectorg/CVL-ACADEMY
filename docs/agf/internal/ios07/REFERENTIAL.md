...
`academy.certification.passed` (BRN-15). Per
...
- Operate this Academy's real event bus literacy: what `events.py`
  ...
  it powers the one real subscriber chain.
- Cross-reference FRK-54 (`docs/frk/frk54/REFERENTIAL.md`) explicitly
  — same underlying mechanism, never re-derived twice.
...
1. `events.py` in-process pub/sub literacy.
...
3. Market-context — `Cvln-ios-v.1`'s real governance/protocol-spec
   corpus, cited honestly as a separate, unconnected system.
...
is connected to `Cvln-ios-v.1`'s real governance layer.
