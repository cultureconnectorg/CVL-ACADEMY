...
| FMS | 12 (FMS-07→18) | REPO_OBSERVED | HIGH | VERIFIED (partial) | `fms-os/fms` (repo réel, cloné) = plateforme business studio (projets/artistes/clients/bookings/services/news/site-config/command-center/integrations/audit-log). Grounds FMS-07 (Studio Ops), FMS-15 (Client/Commercial Ops), FMS-16 (Booking/Planning), FMS-18 (Ecosystem Ops = la couche "os"). FMS-08→14,17 (ingénierie son, mixage, réalisation, A&R, direction créative) = compétences de métier pur, **aucun ancrage code** (normal — ce sont des savoir-faire créatifs, pas des opérations logicielles). |
