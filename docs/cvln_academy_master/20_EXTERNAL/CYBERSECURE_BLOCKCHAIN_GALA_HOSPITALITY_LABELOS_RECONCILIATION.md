...
| GCF-X-04/05/06/07/08 | `NONE`/`PARTIAL` | `CROSS_ECOSYSTEM_ROLE` each | `NEW_CROSS_ECOSYSTEM`, mostly `BLOCKED_PRODUCT_DEPENDENCY` (Kiltikonet/KORA/FREK/Wallet-CVE sides each inherit their own domain's existing block status; GCF-X-08 reuses `MISSIONS_PIPELINES.md` — `EXTEND_EXISTING`). |
