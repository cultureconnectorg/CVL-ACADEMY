...
| KLT-06→08 (Kiltikonet professions) | 3 | `docs/klt/klt06→08/` | `PARTIAL_PACKAGE`, deepened — each file **explicitly self-declares** `STRUCTURAL_STATUS = PARTIAL` (5/7, 6/7, 6/7 compétences construites; the 4 `BLOCKED` competencies — Observatory C5/C6, Network C4, Compliance C4 — stay untouched, never simulated, per `NO_FAKE_OBSERVATORY`/`NO_FAKE_NETWORK`/`NO_FAKE_COMPLIANCE`); module depth on all 17/17 already-`BUILT` modules now enriched to match the KOR-01/02 flagship pattern (2 contrasting `## Exemples` + 3 `## Erreurs fréquentes` per module) |
