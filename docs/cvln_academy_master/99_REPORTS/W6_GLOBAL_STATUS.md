...
| FREK (FRK-01→75) | 75 | `docs/frk/` — **full domain, 75/75 accounted for** | `PARTIAL_PACKAGE` (was `RECONCILED_NOT_BUILT`) — FRK-01 (umbrella, flagship) `PACKAGE_COMPLETE`; 55/75 `MODULE_CONTENT_DRAFTED` (own référentiel each, incl. FRK-71→75 v3 architecture cluster and all market-general clusters); 11/75 `BLOCKED_PRODUCT_DEPENDENCY` (`GAP.md`, genuine repo/spec gaps, no invention); 8/75 `EXTEND_EXISTING` (fold into a sibling or an existing Master Package doc — FRK-05, FRK-45/46, FRK-48/49/50/51/70 — no separate formation, per the reconciliation's own verdict). See `docs/frk/README.md` and `docs/frk/QUALITY_GATES.md`. |
...
| `PARTIAL_PACKAGE` | KOR-03→10 (8), KLT-06→08 (3); FMS-07→18 (9, 1/9 `PACKAGE_COMPLETE`, 8/9 `DRAFTED`); FREK (75, 1/75 `PACKAGE_COMPLETE`, 55/75 `DRAFTED`, 11/75 `BLOCKED`, 8/75 `EXTEND_EXISTING`) |
| `DRAFTED` | KOR-11→15 (5); WAL-20/21/22/23/24/25/26/27/28 (9); CVE-01,03→15 (14); FMS-08/09/10/11/12/13/15/18 (8); FRK (55 — see `docs/frk/README.md`) |
| `BLOCKED` | GMD-34 (product gap); Kiltikonet KLT-09→20 (governance gate); FRK-16/19/21/22/24/39/57/64/65/66/67 (11, genuine product-dependency gaps, `docs/frk/*/GAP.md`) |
| `RECONCILED_NOT_BUILT` | Good Mood/DJ Sayd external-market rows (~51); WAL-01→18 + WAL-X (27); KORA interne/cross (19); Agent Factory cluster (109); CyberSecure+Blockchain+Gala+Hospitality+LabelOS (214); Founder/CEO+Group+Fondation (158); Cross-CVLN XCV-01→56 (~57) |
