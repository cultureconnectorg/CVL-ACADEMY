...
| FREK (FRK-01→75) | 75 | `docs/frk/` (8/75: FRK-01/58/03/06/13/56/59/68) | `PARTIAL_PACKAGE` (corrected this session, was `RECONCILED_NOT_BUILT`) — FRK-01 (umbrella, flagship) `PACKAGE_COMPLETE`; FRK-03/06/13/56/58/59/68 `MODULE_CONTENT_DRAFTED`; 0/8 `BLOCKED`. Grounded directly in the real, re-read `backend/services/frek_core.py` (FRK-01/58/03/06/13/68), the real `FREK_PROOF_MAPPING` headers already on every `docs/kor/` module (FRK-56), and Good Mood's two real, unlinked outbox clients (FRK-59). The other 67 FRK-01→75 candidates (market-general clusters, FRK-71→75 architecture cluster, everything `BLOCKED_PRODUCT_DEPENDENCY`) remain untouched. See `docs/frk/QUALITY_GATES.md`. |
| Agent Factory/AF-X/Laurentia/IOS/Brain/CMD/Intelligent Operations | 109 | none | `RECONCILED_NOT_BUILT` — `AGENT_FACTORY_IOS_BRAIN_CMD_LAURENTIA_RECONCILIATION.md` verdicts exist (including this session's Wave-2 repo-truth delta — `MetaCVLN`/`CVLNAgentfactory`/`Cvln-ios-v.1`/`Laurent.ia` all real), no pedagogical corpus started. ~50 rows are `NEW_EXTERNAL` (market-general, buildable independent of CVLN's own gaps) — the most immediately startable rows in this domain. |
...
|---|---|
| `PACKAGE_COMPLETE` | KOR-01/02, KLT-01→05, FMS-01→06 (pre-existing canon); GMD-21→33 (13/13); WAL-19; CVE-02; FMS-07; FRK-01 |
| `PARTIAL_PACKAGE` | KOR-03→10 (8), KLT-06→08 (3); FMS-07→18 (9, 1/9 `PACKAGE_COMPLETE`, 8/9 `DRAFTED`); FREK slice (8, 1/8 `PACKAGE_COMPLETE`, 7/8 `DRAFTED`) |
| `DRAFTED` | KOR-11→15 (5); WAL-20/21/22/23/24/25/26/27/28 (9); CVE-01,03→15 (14); FMS-08/09/10/11/12/13/15/18 (8); FRK-03/06/13/56/58/59/68 (7) |
| `BLOCKED` | GMD-34 (product gap); Kiltikonet KLT-09→20 (governance gate) |
| `RECONCILED_NOT_BUILT` | Good Mood/DJ Sayd external-market rows (~51); WAL-01→18 + WAL-X (27); KORA interne/cross (19); Agent Factory cluster (109); CyberSecure+Blockchain+Gala+Hospitality+LabelOS (214); Founder/CEO+Group+Fondation (158); FRK-02/04/05/07→75 minus the 8 built (67); Cross-CVLN XCV-01→56 (~57) |
