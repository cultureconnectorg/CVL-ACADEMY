...
| `ORPHAN_SKILL` | 0 | **0** | Chaque `KLT08.SKILL.Cxx` buildable pointe module + assessment + evidence |
| `ORPHAN_MODULE` | 0 | **0** | Chaque module a une compétence unique |
| `ASSESSMENT_WITHOUT_RUBRIC` | 0 | **0** | `KLT08-A01` a `RUBRIC.md` |
| `FAKE_COMPLIANCE` | 0 | **0** | Aucune donnée de conformité simulée pour combler M04 ; `M04` explicitement `BLOCKED`, pas construit |
...
plus important de cette formation (`METHOD_INHERITANCE_VIOLATION = 0`)
est vérifié à chaque niveau du package, pas seulement affirmé une fois
en introduction — cohérent avec `NO_DUPLICATE_CURRICULUM` déjà appliqué
au Master Package.
