...
| `KOR-15` | KORA Network & International Distribution | variable | 100% par formation | `BUILT`, `DRAFTED` (le `REFERENTIAL.md` self-déclare `STATUS = PROPOSED` pour au moins un scénario de cas ; même gabarit compact que `KOR-11`→`14` avant leur reconstruction, non encore engagé) |
