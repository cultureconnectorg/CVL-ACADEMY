...
| `KOR-13` | Creator Partnerships & Cultural Acquisitions | 13 | 13/13 | `BUILT`, `PARTIAL_PACKAGE`, reclassifié depuis `DRAFTED` — gabarit flagship 14-sections, 13/13 modules ; frontières `KOR-05` (handoff, `NO_DUPLICATE_CURRICULUM`) et `KOR-07` (clauses de droits jamais détaillées ici) préservées |
| `KOR-14` | Streaming Product & Experience Operations | 14 | 14/14 | `BUILT`, `PARTIAL_PACKAGE`, reclassifié depuis `DRAFTED` — gabarit flagship 14-sections, 14/14 modules ; frontières `KOR-06` (disponibilité vs ergonomie), `KOR-09` (canal d'acquisition vs expérience in-app), `KOR-12` (performance de contenu vs usage d'interface) et `KOR-05` (expérience creator in-app vs support opérationnel) préservées |
| `KOR-15` | KORA Network & International Distribution | 12 | 12/12 | `BUILT`, `PARTIAL_PACKAGE`, reclassifié depuis `DRAFTED` — gabarit flagship 14-sections, 12/12 modules ; frontières `KOR-07` (handoff droits territoriaux, litige chant traditionnel toujours non résolu, jamais tranché ici), `KLT-07` (coordination de distribution produit vs réseau d'opérateurs internes Kiltikonet, jamais confondus) et `KOR-13` (méthode de partenariat local/institutionnel appliquée, jamais reconstruite) préservées ; `NEEDS_EXPERT_REVIEW = TRUE` sur tout le corpus (droits territoriaux et réglementation locale réels) inchangé |
