...
export default function SpatialBackground({ pathname = "/" }) {
  ...
  const director = useMemo(
    () => directSpatialExperience({ node, scene, reducedMotion: reduced }),
    [node, scene, reduced]
  );
  const motionEnabled = spatialEnabled && !reduced && director.motionIntensity > 0;
  ...
  useEffect(() => {
    ...
    const render = () => {
      currentX += (targetX - currentX) * director.springResponse;
      currentY += (targetY - currentY) * director.springResponse;
      root.style.setProperty("--spatial-x", `${currentX.toFixed(3)}px`);
      root.style.setProperty("--spatial-y", `${currentY.toFixed(3)}px`);
      root.style.setProperty(
        "--spatial-scroll",
        `${(Math.min(scrollY, 900) * director.motionIntensity).toFixed(1)}px`
      );
    ...
    const onPointerMove = (event) => {
      ...
      targetX = nx * director.pointerRangeX;
      targetY = ny * director.pointerRangeY;
    };
    const onScroll = () => {
  ...
  return (
    ...
    >
      <div className="spatial-background__layer spatial-background__layer--far">
        ...
        <div className="spatial-background__clouds spatial-background__clouds--far" />
      ...
      <div className="spatial-background__layer spatial-background__layer--mid">
        ...
        <div className="spatial-background__clouds spatial-background__clouds--near" />
        ...
        <div className="spatial-background__globe">
          <span className="spatial-background__globe-core" />
          <span className="spatial-background__globe-ring spatial-background__globe-ring--a" />
          <span className="spatial-background__globe-ring spatial-background__globe-ring--b" />
        </div>
        ...
        <div className="spatial-background__water-shimmer" />
      </div>
      <div className="spatial-background__layer spatial-background__layer--near">
        <div className="spatial-background__particles" />
