...
export default function FrekProfile() {
  ...
  if (!user) {
    return (
      <div className="px-6 md:px-12 py-10 max-w-7xl" data-testid="frek-profile-page" data-public="true">
        <div className="text-xs uppercase tracking-[0.25em] font-bold text-[--cvln-orange]">{t("frek_profile")}</div>
        <h1 className="font-display font-black text-4xl md:text-5xl tracking-tighter leading-none mt-2">Une identité de parcours, jamais une identité personnelle exposée.</h1>
        <p className="text-[--cvln-ink-2] mt-3 max-w-2xl">FREK relie identité, preuves, signaux de progression et certification dans l’écosystème CVLN Academy. Cette vue publique présente le modèle uniquement : aucun FREK-ID, email, signal personnel ou historique utilisateur n’est chargé.</p>
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          {SIGNALS.map((s) => (
            <div key={s.k} className="cvln-card p-5">
              <div className="mono text-xs text-[--cvln-orange] font-bold">{s.k}</div>
              <div className="text-sm text-[--cvln-ink-2] mt-2">{s.desc}</div>
            </div>
          ))}
        </div>
        <div className="mt-8 flex flex-wrap gap-3">
          <Link to="/register" className="btn-primary">Créer mon identité Academy</Link>
          <Link to="/roadmap" className="btn-outline">Voir la roadmap</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="px-6 md:px-12 py-10 max-w-7xl" data-testid="frek-profile-page">
      <div className="text-xs uppercase tracking-[0.25em] font-bold text-[--cvln-orange]">{t("frek_profile")}</div>
      <h1 className="font-display font-black text-4xl md:text-5xl tracking-tighter leading-none mt-2">{t("frek_profile_p.hero_title")}</h1>
      <p className="text-[--cvln-ink-2] mt-3 max-w-2xl">{t("frek_profile_p.hero_p")}</p>
      ...
      <div className="mt-10 cvln-card p-8 relative overflow-hidden">
        ...
        <div className="relative z-10 grid md:grid-cols-3 gap-6 items-center">
          <div className="md:col-span-2">
            <div className="text-[11px] mono uppercase tracking-[0.25em] text-[--cvln-ink-2] font-bold">{t("frek_profile_p.sovereign_identity")}</div>
          ...
          <div className="text-right">
            <div className="text-xs text-[--cvln-ink-2] mono">{t("frek_profile_p.stade_next_at")} <strong>{prof?.stade_next_at}</strong> {t("frek_profile_p.cc_word")}</div>
            <div className="stage-line mt-2"><div style={{ width: `${prof?.stade_progress_pct ?? 0}%` }} /></div>
            <div className="mt-1 text-xs text-[--cvln-ink-2] mono">{prof?.stade_progress_pct ?? 0}{t("frek_profile_p.stade_progress")}</div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {SIGNALS.map((s) => (
          <div key={s.k} className="cvln-card p-5" data-testid={`signal-${s.k}`}>
            <div className="mono text-xs text-[--cvln-orange] font-bold">{s.k}</div>
            <div className="font-display font-black text-3xl tracking-tighter mt-1">{user.signals?.[s.k] ?? 0}</div>
      ...
      <div className="mt-8 cvln-card p-6">
        ...
        {(prof?.recent_signals ?? []).length === 0 ? (
          <div className="text-sm text-[--cvln-ink-2]">{t("frek_profile_p.no_signals")}</div>
        ) : (
          <div className="space-y-1 mono text-sm">
            {(prof.recent_signals || []).map((s, i) => (
              <div key={i} className="flex items-center justify-between px-2 py-1.5 rounded-lg hover:bg-[--cvln-bg-warm]">
                ...
                <span className="text-[--cvln-ink-2] truncate max-w-[50%]">{s.meta?.module || s.meta?.mission || s.meta?.reason || "—"}</span>
                <span className="text-[--cvln-ink-2] text-xs">{new Date(s.ts).toLocaleString()}</span>
              </div>
            ))}
          </div>
        )}
