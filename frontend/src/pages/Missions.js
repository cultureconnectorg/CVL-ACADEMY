...
function MissionDepthCard({ m, i, primaryIdx, s, reduced, user, t, accept, submit }) {
  ...
  return (
    ...
    >
      <MissionCardBody m={m} s={s} user={user} t={t} accept={accept} submit={submit} />
    </motion.div>
  );
}
...
function MissionCardBody({ m, s, user, t, accept, submit }) {
  return (
    <>
...
export default function Missions() {
  ...
  // fetched above — no ranking data invented for this page. With no
  // session (public view, `mine` always empty) every mission reads as
  // not-yet-accepted, so this simply anchors the first one.
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6">
          {missions.map((m) => (
            <div
              ...
              className="cvln-card p-6 flex flex-col"
            >
              <MissionCardBody m={m} s={status(m.code)} user={user} t={t} accept={accept} submit={submit} />
            </div>
          ))}
        </div>
      )}
