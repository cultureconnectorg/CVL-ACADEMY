...
export default function StakeholderClaim() {
  ...
  useEffect(() => {
    api.get(`/stakeholders/invitations/${code}`)
  ...
  const claim = async () => {
    ...
    try {
      ...
      const target = data.membership.stakeholder_type === "institution" ? "/institution" : "/partner";
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-6 py-12" data-testid="stakeholder-claim-page">
      <div className="cvln-card p-7 max-w-xl w-full">
        <div className="text-xs uppercase tracking-[0.25em] font-bold text-[--cvln-orange]">Accès CVLN Academy</div>
        <h1 className="font-display font-black text-3xl tracking-tight mt-2">Invitation partenaire / institution</h1>
        {error ? (
