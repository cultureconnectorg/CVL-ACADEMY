...
export default function StakeholderPortal({ expectedType }) {
  ...
  if (error) {
    return (
      <div className="px-6 md:px-12 py-10 max-w-4xl">
        <div className="cvln-card p-6">{error}</div>
      </div>
    );
  }
  ...
  return (
    <div
      className="px-6 md:px-12 py-10 max-w-6xl"
      data-testid={`${expectedType}-portal-page`}
    >
        ...
        <div className="mt-4 space-y-2">
          {overview.cohorts.length === 0 ? (
            <div className="text-sm text-[--cvln-ink-2]">
              Aucune cohorte rattachée pour le moment.
            </div>
          ) : (
            overview.cohorts.map((cohort) => (
              <div
                key={cohort.id}
                className="rounded-xl border border-black/5 px-4 py-3 flex flex-wrap items-center justify-between gap-3"
              >
                <div>
                  <div className="font-semibold">{cohort.name}</div>
                  <div className="text-xs text-[--cvln-ink-2]">
                    {cohort.pole || "Tous pôles"}
                  </div>
                </div>
                <div className="text-xs text-[--cvln-ink-2]">
